...
# 1. Obtener todos los usuarios (Clientes y Empleados) con paginación
@router.get("/", dependencies=[Depends(required_admin)])
async def get_all_users(skip: int = 0, limit: int = 50):
    users = await get_all_users_service(skip, limit)
...
async def create_user_by_admin(new_user: UserCreate):
    ...
    # La lógica de la contraseña por defecto y su encriptación ahora la maneja el servicio
    new_id = await create_user_service(new_user.model_dump())
...
# 4. Eliminar un usuario (Suspender / Reactivar cuenta)
@router.patch("/{user_id}/toggle-status", dependencies=[Depends(required_admin)])
async def toggle_user_status(user_id: str):
    # Todo el manejo de MongoDB se delegó a toggle_user_status_service
    new_status = await toggle_user_status_service(user_id)
